  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    when (call.method) {
      ...
      "connect" -> {
        ...
        profileInfo.putString("VpnType", map["type"])
        profileInfo.putString("Alias", map["alias"])
        profileInfo.putString("Cert", map["cert"])
        profileInfo.putInt("MTU", map["mtu"]?.toInt() ?: 1400)

        vpnStateService?.connect(profileInfo, true, activityBinding.activity, map["alias"])
        result.success(true)
      }
      "getCurrentState" -> {
